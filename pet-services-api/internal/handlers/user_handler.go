...
// @Success 201 {object} usecases.AddUserPhotoOutput
// @Failure 400 {object} exceptions.ProblemDetails
...
func (h *UserHandler) AddUserPhoto(c *gin.Context) {
	ctx := c.Request.Context()
	...
	if !exists {
		...
		h.Logger.LogBadRequest(ctx, "UserHandler.AddUserPhoto", problem.Detail, nil)
		c.AbortWithStatusJSON(http.StatusUnauthorized, problem)
		return
	}
	...
	if err != nil {
		...
		h.Logger.LogBadRequest(ctx, "UserHandler.AddUserPhoto", problem.Detail, err)
		c.AbortWithStatusJSON(http.StatusBadRequest, problem)
		return
	}
	defer file.Close()
	...
	input := usecases.AddUserPhotoInput{
		UserID:      userID.(string),
	...
	output, errs := h.UserFactory.AddUserPhoto.Execute(ctx, input)
	if len(errs) > 0 {
		exceptions.HandleErrors(c, errs)
		return
	}
